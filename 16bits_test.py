...
def AVGI(Graph):
    ...
    ## 嘗試是否有建立參照表
    try:
        df = pd.read_csv('RT16.csv')
        RT_table = df.to_numpy()
    except:
        RT = Toollib.APPM_RT16()
        df = pd.DataFrame(RT)
        df.to_csv('RT16.csv', index=False, header=True)
        df = pd.read_csv('RT16.csv')
        RT_table = df.to_numpy()
    ...
    for i in range(Stego.shape[0]):
        for j in range(Stego.shape[1]):
            ...
            G_round = round(Gray)
            ac = (G_round//16 + G_round % var*2) % 16   
...
def Authorize(Graph, Processing_Graph):
    ...
    original = Image.open("image/house.tiff")
    
    ## 檢測圖片是否遭到竄改
    detected_error = 0
    diff_pixels = 0
    undetected_tampered_pixels = []  # 用於記錄沒有被偵測出來但實際被竄改的像素值和驗證碼
    for i in range(Stego.shape[0]):
        for j in range(Stego.shape[1]):
            ...
            if(image1[i,j] != image2[i,j]).any():
                diff_pixels += 1
                if Stego[i,j,0] == 0 and Stego[i,j,1] == 0 and Stego[i,j,2] == 0:
                    undetected_tampered_pixels.append((i, j, G_round, ac,))
    ...
    # 打印沒有被偵測出來但實際被竄改的像素值和驗證碼
    # for i, j, pixel_value, ac in undetected_tampered_pixels:
    #     print(f"Pixel at ({i}, {j}) was tampered but not detected. Pixel value: {pixel_value}, Verification code: {ac}")
